...
def example(request):
    return render(request, 'home/pages/example.html', {})
...
def index(request):
    ...
    try:
        ...
        stockSummary = StockSummary(
            ticker="TSL", last_fetched=datetime.datetime.now())
        stockSummary.save()
    for tweet in cursor:
        # print(tweet)
        ...
        if 'retweeted_status' in tweet._json:
            map_sentimet = perfomSentimentAnalysis(cleanTweet(
        ...
        try:
            ...
            tweet_record = TweetRecord(
                ...
                subjectivity=TextBlob(cleanTweet(
                    tweetToInput)).sentiment.subjectivity,
...
def createHourlyRecord(ticker):
    ...
    list_records_previous_day = TweetRecord.objects.filter(
        stock=stock, tweet_date__hour=(current_hour))
...
def createDailyRecord(ticker):
    ...
    list_records_previous_day = TweetRecord.objects.filter(
        stock=stock, tweet_date__range=[datetime.date.today(), datetime.timedelta(1)])
    # Create hourly record
...
def createMonthlyRecord(ticker):
    ...
    list_records_previous_month = TweetRecord.objects.filter(
        stock=stock, tweet_date__month=month)
    return
