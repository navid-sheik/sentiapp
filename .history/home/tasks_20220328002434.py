...
def get_stock_info():
    ...
    async_to_sync(channel_layer.group_send)(
        'stock', {'type': 'send_joke', 'message':  joke})
...
def mineTweets(self, ticker):
    ...
    cursor = tweepy.Cursor(api.search_tweets, q=keyword,
                           tweet_mode="extended" ,lang="en").items(number_of_tweets)
    ...
    try:
        ...
        stockSummary = StockSummary(
            ticker=ticker, last_fetched=datetime.datetime.now())
        stockSummary.save()
    for tweet in cursor:
        ...
        try:
            ...
            tweet_record = TweetRecord(
                ...
                polarity=TextBlob(cleanTweet(full_text_tweet)
                                  ).sentiment.polarity,
                sentiment=map_sentimet['text_sentiment'],
                subjectivity=TextBlob(cleanTweet(
                    full_text_tweet)).sentiment.subjectivity,
...
def createHourlyRecord(self, ticker):
    ...
    print("The time minus 60 minutes  " + created_time.strftime("%m/%d/%Y, %H:%M:%S"))
    print("The time now  " + current_date.strftime("%m/%d/%Y, %H:%M:%S"))
    list_records_previous_day = TweetRecord.objects.filter(
        stock=stock,tweet_date__gte=created_time, tweet_date__lte=current_date)
    ...
    most_positive = TweetRecord.objects.filter(
        stock=stock, tweet_date__gte=created_time, tweet_date__lte=current_date).order_by('-pos').first().tweet_id
    most_negative = TweetRecord.objects.filter(
        stock=stock, tweet_date__gte=created_time, tweet_date__lte=current_date).order_by('-neg').first().tweet_id
    most_neutral = TweetRecord.objects.filter(
        stock=stock, tweet_date__gte=created_time, tweet_date__lte=current_date).order_by('-neu').first().tweet_id
    most_polarity = TweetRecord.objects.filter(
        stock=stock, tweet_date__gte=created_time, tweet_date__lte=current_date).order_by('-polarity').first().tweet_id
    most_subjectivity = TweetRecord.objects.filter(
        stock=stock, tweet_date__gte=created_time, tweet_date__lte=current_date).order_by('-subjectivity').first().tweet_id
    most_compound = TweetRecord.objects.filter(
        stock=stock, tweet_date__gte=created_time, tweet_date__lte=current_date).order_by('-compound').first().tweet_id
    # Create list of stemmed word
    list_stemmed = list_records_previous_day.values_list(
        'stemmed_text', flat=True)

    # Create hourly record
    list_subjectivity = list_records_previous_day.values_list(
        'subjectivity', flat=True)
    mean_subjectivity = round(mean(list_subjectivity), 6)

    list_polarity = list_records_previous_day.values_list(
        'polarity', flat=True)
    ...
    list_compound = list_records_previous_day.values_list(
        'compound', flat=True)
    mean_compound = round(mean(list_compound), 4)

    # to evaluate
    list_sentiment = list_records_previous_day.values_list(
        'sentiment', flat=True)
    ...
    hourly, created = HourlyRecord.objects.update_or_create(
        stock=stock,
        tweet_date=hour_rounder(current_date),
        ...
        best_tweet_compound=most_compound
...
def get_most_active(self):
    ...
    loop.run_until_complete(channel_layer.group_send(
        name_room, {'type': 'stock_update_most_active', 'message':  response}))
...
def get_most_gainers(self):
    ...
    asyncio.set_event_loop(loop)
    loop.run_until_complete(channel_layer.group_send(
        name_room, {'type': 'stock_update_most_gainers', 'message':  response}))
...
def get_most_losers(self):
    ...
    asyncio.set_event_loop(loop)
    loop.run_until_complete(channel_layer.group_send(
        name_room, {'type': 'stock_update_most_losers', 'message':  response}))


@shared_task(bind=True)
def get_most_volume(self):
    ...
    asyncio.set_event_loop(loop)
    loop.run_until_complete(channel_layer.group_send(
        name_room, {'type': 'stock_update_most_volume', 'message':  response}))
...
def get_most_nasdaq(self):
    ...
    asyncio.set_event_loop(loop)
    loop.run_until_complete(channel_layer.group_send(
        name_room, {'type': 'stock_update_nasdaq_price', 'message':  response}))
...
def cleanTweet(tweet):
    ...
    new_tweet = re.sub(
        "(@[A-Za-z0-9]+)|([^0-9A-Za-z \t])|(\w+:\/\/\S+)", " ", new_tweet)
...
def generateSearchQuery(ticker):
    ...
    query = lower + " OR " + uppper + " OR " + \
        hashtag_lower + " OR " + hashtag_upper + filter
    return query
