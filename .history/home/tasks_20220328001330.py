...
def createHourlyRecord(self, ticker):
    ...
    most_positive = TweetRecord.objects.filter(
        stock=stock, tweet_date__lte=created_time).order_by('-pos').first().tweet_id
    most_negative = TweetRecord.objects.filter(
        stock=stock, tweet_date__lte=created_time).order_by('-neg').first().tweet_id
    most_neutral = TweetRecord.objects.filter(
        stock=stock, tweet_date__lte=created_time).order_by('-neu').first().tweet_id
    most_polarity = TweetRecord.objects.filter(
        stock=stock, tweet_date__lte=created_time).order_by('-polarity').first().tweet_id
    most_subjectivity = TweetRecord.objects.filter(
        stock=stock, tweet_date__lte=created_time).order_by('-subjectivity').first().tweet_id
    most_compound = TweetRecord.objects.filter(
        stock=stock, tweet_date__lte=created_time).order_by('-compound').first().tweet_id
