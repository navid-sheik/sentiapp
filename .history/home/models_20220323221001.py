...
class TweetRecord(models.Model):
    ...
    raw_text = models.TextField("raw_text",blank=True)
    processed_text = models.TextField("raw_text", blank=True)
    stemmed_text =  models.TextField("stemmed_text", blank=True)
    ...
    def __str__(self) -> str:
        return self.stock.ticker  + " "  + str(self.tweet_id) 

class HourlyRecord (models.Model):
    stock = models.ForeignKey(StockSummary, on_delete=models.CASCADE, default="TSL")
    tweet_date =  models.DateTimeField("date_fetched", null=True)
    overall_polarity =   models.DecimalField(max_digits=20, decimal_places=6)
    overall_stemmed_text =  models.TextField("overall_stemmed_text", blank=True)
    ...
    def __str__(self) -> str:
        return self.stock.ticker  + " "  + self.tweet_date.strftime("%d %B, %Y - %H:%M" )
