...
def extendTimings(orginalTimings, lenTasks ):
    if lenTasks == 0 :
        return orginalTimings
    else:
        return orginalTimings + (lenTasks * 20)
...
def startMining(request, ticker_id):
    ...
    namesMining =   "mining-tweets-for-" 
    tasksMining = PeriodicTask.objects.filter(name__startswith =namesMining)

    namesHourly =   "create-hourly-record-for-" 
    tasksHourly = PeriodicTask.objects.filter(name__startswith =namesHourly)
    ...
    schedule, createdMiner = IntervalSchedule.objects.get_or_create(
        every=extendTimings(360, len(tasksMining)), period=IntervalSchedule.SECONDS)
    ...
    scheduleHourly, createdHourly = IntervalSchedule.objects.get_or_create(
        every=extendTimings(3600, len(tasksHourly)), period=IntervalSchedule.SECONDS)
