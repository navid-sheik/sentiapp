...
def update_sentiment_24Hours(self, ticker_id):
    ...
    res =  json.dumps(rngData, indent = 4, sort_keys = True, default = str)

    name_room  = 'stock_%s' % stock_ticker
    loop =  asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(channel_layer.group_send(name_room, {'type': 'sentiment_update', 'sentiment' :   res}))
